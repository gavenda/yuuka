...
const groups = computed<AccountGroup[]>(() => {
	...
	for (const [typeId, accounts] of byType) {
		groups.push(toGroup(typeId || 'untyped', accounts[0]!.typeName ?? 'Uncategorised', accounts));
	}
		...
		<ModalDialog :open="dialogOpen" :title="editing ? 'Edit account' : 'New account'" @close="dialogOpen = false">
			<form class="space-y-4" @submit.prevent="save">
				...
				<div>
					...
					<p class="mt-1 text-xs text-slate-500 dark:text-slate-400">The balance before any transaction below was recorded.</p>
				</div>
