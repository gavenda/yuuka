	...
	<div class="py-3">
		<div class="flex items-baseline justify-between gap-3">
			<div class="flex min-w-0 items-center gap-2">
				...
				<span class="truncate text-sm font-medium text-on-surface">{{ entry.name }}</span>
			</div>

			<span class="tabular shrink-0 text-sm text-on-surface-variant">
				{{ displayMoney(entry.actual, currency) }}
				<span v-if="entry.planned > 0" class="text-outline">/ {{ displayMoney(entry.planned, currency) }}</span>
			</span>
		</div>

		<!-- Track is a light step of the same ramp so the state reads across the whole bar. -->
		<div class="mt-2 h-2 overflow-hidden rounded-full bg-surface-container-highest">
		...
		<ul v-if="entry.children.some((child) => child.actual > 0)" class="mt-1.5 space-y-0.5 pl-4">
			<li v-for="child in entry.children.filter((c) => c.actual > 0)" :key="child.categoryId" class="flex justify-between gap-2 text-xs">
				<span class="truncate text-on-surface-variant">{{ child.name }}</span>
				<span class="tabular shrink-0 text-on-surface-variant">{{ displayMoney(child.actual, currency) }}</span>
			</li>
		</ul>

		<p class="mt-1.5 flex items-center gap-1.5 text-xs" :class="over ? 'text-error' : 'text-on-surface-variant'">
